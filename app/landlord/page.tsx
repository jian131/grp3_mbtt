    ...
    <div className="min-h-screen pt-24 pb-20 px-4 md:px-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {[
            { label: 'Lượt Xem Tin', val: '1,204', sub: '30 ngày qua', icon: Eye, color: 'text-purple-400', bg: 'bg-purple-500/10' },
            { label: 'Khách Quan Tâm', val: '45', sub: 'Độ quan tâm cao', icon: Users, color: 'text-cyan-400', bg: 'bg-cyan-500/10' },
            { label: 'Tiềm Năng Tăng Giá', val: '+15%', sub: 'So với năm ngoái', icon: ArrowUpRight, color: 'text-green-400', bg: 'bg-green-500/10' }
          ].map((stat, idx) => (
            ...
            <form className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  ...
                  <input type="text" className="w-full bg-white/5 border-b border-white/10 focus:border-cyan-500 p-3 text-white outline-none transition-all" defaultValue="45 Lê Thánh Tôn, Q.1" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-400 mb-2">Diện Tích (m²)</label>
                  <input type="number" className="w-full bg-white/5 border-b border-white/10 focus:border-cyan-500 p-3 text-white outline-none transition-all" defaultValue="80" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-400 mb-2">Mặt Tiền (m)</label>
                  <input type="number" className="w-full bg-white/5 border-b border-white/10 focus:border-cyan-500 p-3 text-white outline-none transition-all" defaultValue="5" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-400 mb-2">Số Tầng</label>
                  <input type="number" className="w-full bg-white/5 border-b border-white/10 focus:border-cyan-500 p-3 text-white outline-none transition-all" defaultValue="2" />
                </div>
              </div>
              ...
              <div className="p-8 rounded-2xl bg-gradient-to-r from-blue-900/20 to-cyan-900/20 border border-cyan-500/30 flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                  ...
                  <div className="text-3xl md:text-5xl font-black text-white tracking-tight">
                    45 - 50 <span className="text-lg font-medium text-gray-400">Triệu / Tháng</span>
                  </div>
                  <p className="text-sm text-gray-500 mt-2 max-w-md">
                    Dựa trên 12 tài sản tương đồng trong bán kính 500m.
